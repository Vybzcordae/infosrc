...
if(checkUserSession($db) !== FALSE && !empty($_POST["txt-message"]) && !empty($_GET["room_id"])){
	$blackList = ["mbolo", "machende", "nyini", "nyelo", "gm", "fuck", "nigga", "faggot", "chitsiru", "zoba"];
	...
	if(mysqli_num_rows($user_query) > 0){
			...
			if(mysqli_num_rows($room_query) > 0){
				...
				if(0 < count(array_intersect(array_map('strtolower', explode(' ', $message)), $blackList))){
					$sendStatus = array("success" => false, "message" => "Your text contains the banned word(s)");
				}else{
