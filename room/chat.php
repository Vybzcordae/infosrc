...
if(!empty($_GET["room_id"])){
	...
	$query = mysqli_query($db, "select * from chat_room where room_id='{$room_id}'"); //$_HOME_FILE --> /config/value.php
	if(mysqli_num_rows($query) > 0){
		...
		echo "<script>alert('No Room with this Id !!'); window.location='home.php'</script>";
	}
}else {
	echo "<script>alert('No inputs !!'); window.location='home.php'</script>";
}
?>
                                ...
                                <div class="chat-message-form">
                                    ...
                                    <div class="form-group">
										<form id="send-message">
											<textarea style="resize: none;" class="form-control message-input" id="txt-message" name="txt-message" placeholder="Enter message text" rows="5"></textarea>
											<button id="txt-message" class="send">Send</button>
										</form>
                                    </div>
				...
				r.forEach(function(m){
				  if(m.owner){
					  ...
					  var temp = '<div class="chat-message right">' +
						'<img class="message-avatar" src="../Admin/php/images/<?= $profilePicture ?>" alt="">' +
						'<div class="message">' +
					...
					'</div>';
					  ...
					  chat_temp += temp.replace("<?= $profilePicture ?>", m.profilePicture).replace("${name}", "You").replace("${message}", m.message).replace("${time}", m.time_ago)
				  } else {
					  //left messages
...
$("#request_join").on('click',(function(e) {
	$.ajax({
		...
		success: function(r) {
			if(r.success){
				$('#request_join').text("Pending approval...").prop('disabled', true)
			} else {
