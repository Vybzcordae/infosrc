...
if(!empty($tool_search)){
	...
	$query = mysqli_query($conn, "select * from uploads where name='{$tool_search}'"); //$_HOME_FILE --> /config/value.php
	if(mysqli_num_rows($query) > 0){
		//$_SESSION["current_room_id"] = $room_id;
		$tools_data = mysqli_fetch_array($query);
					
	} else {
		echo "<script>alert('No Item with this name !!'); window.location='Tools.php'</script>";
	}
}else {
	echo "<script>alert('Please enter Item name !!'); window.location='Tools.php'</script>";
}
?>
        ...
        <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
            ...
            <nav id="navbar" class="navbar">
                <ul>
                    ...
                    <li><a href="posts.php">Tools</a></li>
