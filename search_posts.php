...
$post_search = mysqli_real_escape_string ($con, $_POST['post_search']);
if(!empty($post_search)){
	
	$query = mysqli_query($con, "select * from posts where topic='{$post_search}'"); //$_HOME_FILE --> /config/value.php
	if(mysqli_num_rows($query) > 0){
		//$_SESSION["current_room_id"] = $room_id;
		$posts_data = mysqli_fetch_array($query);
					
	} else {
		echo "<script>alert('No Post with this question !!'); window.location='posts.php'</script>";
	}
}else {
	echo "<script>alert('Please enter post question !!'); window.location='posts.php'</script>";
}
?>
